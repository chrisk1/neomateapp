...
// Analytics
	...
	$("#babyweightform").submit(function() {
		Keyboard.hide();
		return false;
	});
...
*/
	...
	// EMERGENCY DRUGS
		var adrenalinedose1 = Math.round((babyweight*0.2)*10)/10;
...
/* START NEONATAL FLUIDS SECTION */
		...
		if (babyweight != "")
		{
			if (tifluid != "")
			{
				...
				if (dailytotalfluidreq < 4) {
					$("#fluidoutput").html("<b>Daily total fluid requirement</b><br>= This amount of fluid is lower than usually given on NICU. Please check the fluid amount carefully above.<br><br>");
				}else {
...
// array of drug names, usual delivery units and max concentration in syringe
	...
	if (this.drugtotalmg() < 0.001) {
		this.advisoryerror = 1;
		console.log("Drug total is 0 - WARNING!");
		this.advisorymsg = this.advisorymsg + "Drug dose added to syringe is probably too small. Please check your entry carefully."; 
	}
